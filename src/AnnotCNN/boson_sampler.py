...
class ConvolutionalInterferometerBuilder:
    ...
    def create_circuit(self, m: int, parameters: list = None) -> pcvl.Circuit:
        ...
        # Build sub-circuits:
        U1 = pcvl.GenericInterferometer(m, lambda idx: pcvl.BS() // pcvl.PS(pcvl.P(f"theta1_{idx}")),
                                        shape=pcvl.InterferometerShape.TRIANGLE)
        U2 = pcvl.GenericInterferometer(m, lambda idx: pcvl.BS() // pcvl.PS(pcvl.P(f"theta2_{idx}")),
                                        shape=pcvl.InterferometerShape.TRIANGLE)
        U3 = pcvl.GenericInterferometer(m, lambda idx: pcvl.BS() // pcvl.PS(pcvl.P(f"theta3_{idx}")),
                                        shape=pcvl.InterferometerShape.TRIANGLE)
        Conv1 = pcvl.GenericInterferometer(m, lambda idx: pcvl.BS() // pcvl.PS(alpha_params[idx]),
                                           shape=pcvl.InterferometerShape.TRIANGLE)
        Conv2 = pcvl.GenericInterferometer(m, lambda idx: pcvl.BS() // pcvl.PS(beta_params[idx]),
                                           shape=pcvl.InterferometerShape.TRIANGLE)
...
class BosonSampler:
    def __init__(self, m: int, n: int, postselect: int = None, backend: str = "SLOS", session=None,
                 builder: InterferometerBuilder = None, cache_enabled=False, cache_directory="results/cache"):
        """
    ...
    def embed(self, data_tensor, n_samples: int):
        ...
        if getattr(self, "cache_enabled", False):
            ...
            if os.path.exists(cache_file):
                ...
                try:
                    ...
                    # Fallback for old pickle cache files - delete and regenerate
                    os.remove(cache_file)
                    print(f"Removed incompatible cache file: {cache_file}")
        ...
        if isinstance(self.builder, ConvolutionalInterferometerBuilder):
            ...
            flat = data_tensor.flatten()
            #print(f"\n ---- \n Nb of parameters {self.nb_parameters} \n ---- ")
            if flat.shape[0] > self.nb_parameters:
                raise ValueError("Input tensor too large for the number of modes/photons")
        ...
        if getattr(self, "cache_enabled", False):
            torch.save(feature_vec, cache_file)
